...
function expandBare(correct, claim, topic, seed) {
    ...
    if (/автор:/i.test(correct)) {
        const authors = ["Эрик Рис", "Александр Остервальдер", "Дэйв МакКлюр", "Роб Фитцпатрик"];
        return fin(`${c}. Автор: ${pick(authors, seed)} (неверная атрибуция для этого метода).`);
    }

    // Topic-specific unique expansions — claim always inside, many variants
    const bank = {
        Метрики: [
            `${c} — так иногда называют соседний KPI, но формула и управленческий смысл другие.`,
            `${c}: похожий индикатор из продуктовой аналитики, однако считает не то поведение.`,
            `Берут ${c} вместо нужной метрики и оптимизируют охват или vanity-рост.`,
            `${c} выглядит убедительно на дашборде, но не отвечает на вопрос этого определения.`
        ],
        "Финансовая модель": [
            `${c} — другой тип отчёта/статьи: путают P&L, cash flow или баланс.`,
            `${c}: в модели так часто подписывают блок, хотя смысл и состав строк другие.`,
            `${c} смешивает начисления и живые деньги либо OpEx и COGS.`,
            `Считать ${c} ответом на вопрос — значит выбрать неверный финансовый срез.`
        ],
        "Юнит-экономика": [
            `${c} не является юнит-метрикой: нет связки выручки, затрат и одного клиента.`,
            `${c} — соседний расчёт, но без LTV/CAC/маржи на юнит экономика не сходится.`,
            `Если ответить «${c}», путают юнит с оперметрикой или рыночным анализом.`,
            `${c} не показывает прибыльность одного клиента с учётом привлечения и удержания.`
        ],
        JTBD: [
            `${c} — не Jobs-to-be-Done: фокус не на работе, которую «нанимают» продукт выполнить.`,
            `${c} ближе к процессу разработки/дизайна, чем к Job Statement и switch.`,
            `Свести JTBD к «${c}» — потерять Forces of Progress и контекст работы.`,
            `${c} не объясняет, какую работу пользователь пытается сделать в ситуации прогресса.`
        ],
        CustDev: [
            `${c} — не Customer Development: нет цикла гипотеза → интервью → обучение.`,
            `${c} может быть инструментом рядом, но автором и сутью CustDev не является.`,
            `Свести CustDev к «${c}» — подменить проверку гипотез узкой активностью.`,
            `${c} не описывает методологию Бланка по валидации продукта с клиентами.`
        ]
    };

    let out = pick(bank[topic] || bank["Метрики"], seed);

    // If correct has 2+ sentences, try to add a second clause unique to claim
    if (parts.length >= 2 && out.length < clen * 0.75) {
        const extras = [
            ` В отличие от верного определения, здесь нет того же критерия успеха.`,
            ` Для команды это ведёт к другому next step в приоритезации.`,
            ` На практике такой ответ путают с правильным из‑за похожей терминологии.`
        ];
        // Make unique with claim snippet
        out = fin(
            out.replace(/[.!?]$/, "") +
                pick(extras, seed + 3).replace(/\.$/, "") +
                ` («${c.slice(0, 28)}»).`
        );
    }

    if (out.length > clen * 1.35) {
        out = fin(out.slice(0, Math.floor(clen * 1.2)).replace(/\s+\S*$/, ""));
    }
    return out;
}
...
// audit
const BAD =
    /next step по|В когортах «|На weekly «|Ломает Mom Test:|Заложить |Если взять |Если метод —|Берут [А-ЯA-Z]|Опора на |Считать как |persona\/feature|Юнит сходится «на бумаге»|Метрика считается иначе|не совпадает с определением|Формула смешивает базу|Так путают похожие/i;
